...
int TMainForm::CreateNewBus(int ID) {
	...
	if (busWorker->start(xVec, yVec, UpdatePositionBus->Interval, 1000))
		...
		return BusesListComboBox->Items->Add(L"Bus #" + IntToStr(busSeries->Tag));
	else
        return -1;
}
...
{
    // Time Update
	if (StartButton->Enabled == false) TimeLabel->Caption =
									   IntToStr((int) ((clock() - startModelingTime) / CLOCKS_PER_SEC)) + L" sec";
	...
	for(int i = 0; i < CountOfBusesEdit->Text.ToInt(); i++) {
		if (busWorkerVec[i]->isDataUpdate()) {
			SetPosition(busSeriesVec[i], busWorkerVec[i]->getXValue(), busWorkerVec[i]->getYValue());
	...
	if (isPointSelect) {
		...
		for(int i = 0; i < busWorkerVec.size(); i++) {
			...
			if (time < bestTimeToArrival || bestTimeToArrival == 0) bestTimeToArrival = time;
		}
		BusStopInfoMemo->Lines->Clear();
		BusStopInfoMemo->Lines->Add(L"Bus stop number: " + IntToStr(indexSelectedPoint));
		BusStopInfoMemo->Lines->Add(L"Time before the arrival of the bus: " + IntToStr(bestTimeToArrival) + L" sec");
	}
	// Если ни одна остановка не выбрана, BusStopInfoMemo просто поддерживается не заполненым
	else BusStopInfoMemo->Lines->Clear();
	...
	if (selectedBusWorker != NULL) {
        BusInfoMemo->Clear();
        BusInfoMemo->Lines->Add(L"Bus number: " + IntToStr(selectedBusWorker->getID()));
		BusInfoMemo->Lines->Add(L"Current speed: " + FloatToStr(selectedBusWorker->getCurrentSpeed()));
		int numberNextBusStop = selectedBusWorker->getCurrentSiteNumber();
		BusInfoMemo->Lines->Add(L"Next bus stop: " + IntToStr(numberNextBusStop));
		BusInfoMemo->Lines->Add(L"Time to arrival at next bus stop: " +
								IntToStr(selectedBusWorker->getTimeToArrival(numberNextBusStop)) + L" sec");
		BusInfoMemo->Lines->Add(L"Service status: " + BoolToStr(selectedBusWorker->getServiceable(), true));
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::StopButtonClick(TObject *Sender)
{
	...
	std::for_each( busWorkerVec.begin(), busWorkerVec.end(), std::default_delete<BusWorker>() );
	std::for_each( busSeriesVec.begin(), busSeriesVec.end(), std::default_delete<TPointSeries>() );
...
{
	if (indexSelectedPoint != -1) BusStopSeries->Pointer->operator [] (indexSelectedPoint)->Color = clLime;
	indexSelectedPoint = -1;
	BusStopInfoMemo->Lines->Clear();
	isPointSelect = false;
}
...
void __fastcall TMainForm::BusStopSeriesClickPointer(TCustomSeries *Sender, int ValueIndex,
          int X, int Y)
{
    // При выборе остановки, необходимо снять выделение с предыдущей выбранной
	if (indexSelectedPoint != -1) ResetSelectedPoint_MainChartDblClick(this);
...
void __fastcall TMainForm::BusSeriesGetMarkText(TChartSeries *Sender, int ValueIndex,
...
void TMainForm::SelectBusInBusesList(int index) {
	...
	int numberBus = selectedBusStr.SubString(indexStartNumber, selectedBusStr.Length() - (indexStartNumber - 1) ).ToInt();

	for(int i = 0; i < busSeriesVec.size(); i++) {
		if (busWorkerVec[i]->getID() == numberBus) selectedBusWorker = busWorkerVec[i];
